...
from scannetpp import SEMANTIC_CAT_SCANNET_PP, SEMANTIC_INSTANCE_CAT_SCANNET_PP
...
class VisualizationScannetpp:
    ...
    def gtviz(self, gt_data, specific = False): # pending
        ...
        for i in range(0, n_label.shape[0]):
            ...
            limit -= 1
            name.append(class_names[sem_label[np.where(ins_label==n_label[i])][0]])
    ...
    def vizmask2d(self, mask2d_path, specific = False):
        ...
        instance = dic['ins']
        instance = [torch.tensor(rle_decode(ins)) for ins in instance]
        ...
        for i in range(0, len(instance),1):
            tt_col[instance[i] == 1] = pallete[i]
            if specific and limit > 10: # be more specific but limit 10 masks (avoiding lag)
                limit -= 1
                tt_col_specific = self.color.copy()
                tt_col_specific[instance[i] == 1] = pallete[i]
                self.vis.add_points(f'2D lifted mask: ' + str(i) + '_' + str(conf2d[i].item())[:5], self.point, tt_col_specific, point_size=20, visible=True)

        self.vis.add_points(f'2D lifted mask: ' + str(len(instance)), self.point, tt_col, point_size=20, visible=True)
        print('---Done---')        
...
if __name__ == "__main__":
    ...
    ## 4
    check_2dviz = True
    # mask2d_path = '../freevocab_exp_scannetpp/version_dp/mask2d_lifted/' + scene_id + '.pth'
    mask2d_path = '../freevocab_exp_scannetpp/version_dp_256_fast/mask2d_lifted/' + scene_id + '.pth'
    ## 5
    check_finalviz = False
    agnostic_path = '../freevocab_exp_scannetpp/version_dp/freevocab_results/' + scene_id + '.pth'
    ...
    if check_finalviz:
        VIZ.finalviz(agnostic_path, specific = True, vocab = False)
