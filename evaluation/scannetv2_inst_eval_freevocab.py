...
import numpy as np
from loader3d.scannet200 import (
...
from evaluation.instance_eval_util import get_instances
        ...
        for pred in preds:
            if self.use_label:
                ...
                if label_id not in self.id2label:
                    continue
                target = torch.where(pred['vocab_score'] > 0.4)[0].cpu()
                label_names = [self.id2label[tt.item() + 1] for tt in target]
                # label_name = self.id2label[label_id] # (list of corresponding labels)
            else:
