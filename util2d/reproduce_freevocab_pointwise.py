...
class OSM():
    ...
    def query_llm(self, representatives):
        ...
        generated_output = self.class_generator.generate_1n_avg_language(
                                ...
                                input_ids=torch.cat(input_ids_batch).cuda(),
                                attention_mask=torch.cat(attention_mask_batch).cuda(),
                                dataset_type="any",
                                max_new_tokens=16,
                                num_beams=1,
                                return_dict_in_generate=True,
...
class FreeVocab_Reproduce:
    def __init__(self, cfg, class_names):
        # OpenAI CLIP
        self.device = "cuda:0"
        self.model = OSM(osm_checkpoint = '/mnt/cpfs02/home/chenhui.yang/Any3DIS_unofficial/weighs/osm_final.pt')
    ...
    def pointwise_aggregator(self, scene_id, cfg):
        ...
        for i in range (len(self.pcd_list)):
            if len(self.pcd_list[i]['masks']) > 0:
                self.pcd_list[i]['masks'] = torch.stack(self.pcd_list[i]['masks'])
            else:
                self.pcd_list[i]['masks'] = torch.zeros((0, 968, 1296), dtype=torch.bool)
        # Image Alignment
        self.align_majority()
