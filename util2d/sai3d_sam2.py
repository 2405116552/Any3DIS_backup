...
class SAI3D_SAM_L2:
    ...
    def generate3dproposal(self, scene_id, cfg):
        ...
        groups = []
        for ii in range(0, len(self.pcd_list),1):
            group, _ = self.region_growing(self.pcd_list, ii, ii, spp, n_spp, self.n_points, sieve_of_spp, visi=visi, reca = recall, simi=simi, iterative=True)
            if len(group) == 0:
                continue
            groups.append(group)
        end_agg_time = time.time()
        self.agg_accu_time += end_agg_time - start_agg_time
        print('Agg: ', self.agg_accu_time)

        if len(groups) == 0:
            return None, None
        groups = torch.cat(groups,0)
        groups = groups.to(torch.int64).to(self.device)
        
        merged_instance = groups.clone().cuda()
        iou_levels = [0.9, 0.8, 0.7]
        for iou_level in iou_levels:
            iou_matrix, _, recall_matrix = compute_relation_matrix_self(merged_instance, spp, sieve_of_spp)
            adj_mat = iou_matrix > iou_level
            adj_mat = adj_mat | adj_mat.T 
            connected_components = find_connected_components(adj_mat)
            M = len(connected_components)

            merged_instance_new = torch.zeros((M, n_spp), dtype=torch.int8, device=self.device)
            for i, cluster in enumerate(connected_components):
                merged_instance_new[i] = merged_instance[cluster].sum(0)
            merged_instance = merged_instance_new.clone()

        groups = merged_instance.clone().to(torch.int64).cpu()
        spp = spp.cpu()
        ...
        print('Total: ', overall_end_time - self.overall_start_time)
        return proposals_pred, None, None, None
