...
class SAM_L2:
    ...
    def generate3dproposal(self, scene_id, cfg):
        ...
        ###NOTE: TIME - accumulate across all FPS iterations
        self.overall_start_time = time.time()
        self.seg_accu_time = 0
        self.track_accu_time = 0
        self.lift_accu_time = 0
        self.refine_accu_time = 0
        self.sam2_init_accu_time = 0
        self.sam2_prompt_accu_time = 0
        ...
        while END_FLAG == False:
            ...
            for spp_id in tqdm(spp_ids):
                ###NOTE: Ablation: Fill visited conditions & better visualization & fewer masks
                ...
                for peak_id in peaks:
                    ...
                    start_sam2_prompt_time = time.time()
                    ...
                    while RANSAC_Rounds > 0:
                        ...
                        self.seg_accu_time += end_seg_time - start_seg_time

                        if scores[0] > maximum_score:
                            init_mask_prompts = masks_prompts
                            maximum_score = scores[0]
                        RANSAC_Rounds -= 1
                    end_sam2_prompt_time = time.time()
                    self.sam2_prompt_accu_time += end_sam2_prompt_time - start_sam2_prompt_time
                    ...
                    def forward_sam2(images, mappings_video, video_path = None):
                        ...
                        start_sam2_init_time = time.time()
                        ...
                        end_sam2_init_time = time.time()
                        self.sam2_init_accu_time += end_sam2_init_time - start_sam2_init_time
                        ...
                        self.track_accu_time += end_track_time - start_track_time

                        mask3d =[(0, torch.zeros((n_spp), dtype=torch.int8, device=self.device))]
                        
                        start_lift_time = time.time()
                        ...
                        for view_id in range(len(video_segments)):
                            ...
                            if len(target_spp) < 1:
                                ...
                                if valid_spp.sum() > 0:
                                    ...
                                    mask3d.append((track_id, torch.zeros((n_spp), dtype=torch.int8, device=self.device)))
                        end_lift_time = time.time()
                        self.lift_accu_time += end_lift_time - start_lift_time
                    ...
                    self.refine_accu_time += end_refine_time - start_refine_time
                    ...
                    print('\nSeg: ', self.seg_accu_time, '\nTrack: ', self.track_accu_time, '\nLift: ',self.lift_accu_time, '\nRefine: ', self.refine_accu_time, '\n')
            overall_end_time = time.time()
            print('Overall Time: ', self.overall_start_time, overall_end_time)
        # Return results
        overall_end_time = time.time()
        total_time = overall_end_time - self.overall_start_time if hasattr(self, 'overall_start_time') else 0
        sam2_segtrack = getattr(self, 'seg_accu_time', 0) + getattr(self, 'track_accu_time', 0)
        sam2_init = getattr(self, 'sam2_init_accu_time', 0)
        sam2_prompt = getattr(self, 'sam2_prompt_accu_time', 0)
        sam2_total = sam2_segtrack + sam2_init + sam2_prompt
        postprocess_time = getattr(self, 'lift_accu_time', 0) + getattr(self, 'refine_accu_time', 0)
        other_time = total_time - sam2_total - postprocess_time if total_time > 0 else 0
        print(f"\n========== SCENE {scene_id} TIME SUMMARY ==========")
        print(f"  Total generate3dproposal time: {total_time:.2f}s")
        print(f"  SAM2 total time:               {sam2_total:.2f}s")
        print(f"    ├─ seg (image_predict):      {getattr(self, 'seg_accu_time', 0):.2f}s")
        print(f"    ├─ track (propagate_video):  {getattr(self, 'track_accu_time', 0):.2f}s")
        print(f"    ├─ init_state/reset/add_*:   {sam2_init:.2f}s")
        print(f"    └─ set_image + prompt prep:  {sam2_prompt:.2f}s")
        print(f"  Post-process (lift+refine):    {postprocess_time:.2f}s  (lift={getattr(self, 'lift_accu_time', 0):.2f}s, refine={getattr(self, 'refine_accu_time', 0):.2f}s)")
        print(f"  Other overhead:                {other_time:.2f}s")
        print(f"=================================================\n")
