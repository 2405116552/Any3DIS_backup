...
class OpenVocab_Reproduce_PointWise:
    ...
    def point_aggregator(self, scene_id):
        '''
            CLIP feature aggregator using weighted Superpoints
        '''
        pcd_list = self.pcd_list
        print("-------Feature Aggregating-------")
        feature_bank = torch.zeros((self.points.shape[0], self.clip_dim), dtype = torch.float64, device = self.device)
        mappings = []
        images = []
        ...
        for (obj_id, inst) in enumerate(tqdm(self.instance3d)):
            ...
            for v in range(len(images)):
                ...
                for round in range(3):
                    ...
                    cropped_3d.append(self.instance3d[obj_id])
                    # if i < 10 and rr == 3: 
        ...
        #             cropped_3d.append(masks3D[inst])
    ...
    def refine_openvocab(self, scene_id, cfg, genfeature = True):
        ...
        if genfeature:
            ...
            self.storage2d = []
            mask2d_bank = dic['mask2d_bank']
            id_bank = dic['id_bank']
            for i in range(len(mask2d_bank)):
                storage_entry = [{'mask2d': mask2d_bank[i], 'im_mask2d_id': id_bank[i]}]
                self.storage2d.append(storage_entry)
            self.feature_bank = self.point_aggregator(scene_id)
        else:
    ...
    def get_final_instance(self, scene_id, cfg):
        ...
        #NOTE: Superpoint pooling strategy - reduce mem
        ...
        inst_class_scores = torch.einsum("kn,nc->kc", self.instance3d.float().cpu(), predicted_class.float().cpu()).cuda()  # K x classes
        inst_class_scores = inst_class_scores / self.instance3d.float().cuda().sum(dim=1)[:, None]  # K x classes
        ...
        labels = (
            ...
            .unsqueeze(0)
            .repeat(self.instance3d.shape[0], 1)
            .flatten(0, 1)
        )
